...
export class ScheduleService {
  ...
  getDepartmentLookup = (input: LookupRequestDto, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<LookupDto<string>>>({
      method: 'GET',
      url: '/api/app/schedules/department-lookup',
  ...
  getList = (input: GetSchedulesInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<ScheduleWithNavigationPropertiesDto>>({
      method: 'GET',
      url: '/api/app/schedules',
      params: { filterText: input.filterText, code: input.code, name: input.name, dateFromMin: input.dateFromMin, dateFromMax: input.dateFromMax, dateToMin: input.dateToMin, dateToMax: input.dateToMax, note: input.note, departmentId: input.departmentId, scheduleDetailId: input.scheduleDetailId, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
  ...
  getListAsExcelFile = (input: ScheduleExcelDownloadDto, config?: Partial<Rest.Config>) =>
    this.restService.request<any, Blob>({
      ...
      params: { downloadToken: input.downloadToken, filterText: input.filterText, code: input.code, name: input.name, dateFromMin: input.dateFromMin, dateFromMax: input.dateFromMax, dateToMin: input.dateToMin, dateToMax: input.dateToMax, note: input.note, departmentId: input.departmentId, scheduleDetailId: input.scheduleDetailId },
    },
    { apiName: this.apiName,...config });
