...
export class TitleService {
  ...
  getList = (input: GetTitlesInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<TitleDto>>({
      method: 'GET',
      url: '/api/app/titles',
      params: { filterText: input.filterText, code: input.code, name: input.name, note: input.note, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
