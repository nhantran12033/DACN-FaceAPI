import type { GetScheduleFormatsInput, ScheduleFormatCreateDto, ScheduleFormatDto, ScheduleFormatUpdateDto } from './models';
...
export class ScheduleFormatService {
  ...
  getList = (input: GetScheduleFormatsInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<ScheduleFormatDto>>({
      method: 'GET',
      url: '/api/app/schedule-formats',
      params: { filterText: input.filterText, name: input.name, dateMin: input.dateMin, dateMax: input.dateMax, fromHoursMin: input.fromHoursMin, fromHoursMax: input.fromHoursMax, toHoursMin: input.toHoursMin, toHoursMax: input.toHoursMax, note: input.note, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
