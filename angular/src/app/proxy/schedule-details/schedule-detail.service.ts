import type { GetScheduleDetailsInput, ScheduleDetailCreateDto, ScheduleDetailDto, ScheduleDetailUpdateDto, ScheduleDetailWithNavigationPropertiesDto } from './models';
...
export class ScheduleDetailService {
  ...
  getList = (input: GetScheduleDetailsInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<ScheduleDetailWithNavigationPropertiesDto>>({
      method: 'GET',
      url: '/api/app/schedule-details',
      params: { filterText: input.filterText, name: input.name, fromDateMin: input.fromDateMin, fromDateMax: input.fromDateMax, toDateMin: input.toDateMin, toDateMax: input.toDateMax, note: input.note, scheduleFormatId: input.scheduleFormatId, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
